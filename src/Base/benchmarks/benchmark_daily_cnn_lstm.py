...
SHARED = load_module("daily_mlp_benchmark_shared_for_cnn_lstm", SCRIPT_DIR / "benchmark_daily_mlp.py")
CNN_LSTM_MODEL = load_module("cnn_lstm_baseline_module", MODELS_DIR / "cnn_lstm_baseline.py")
PTL_CORE = SHARED.PTL_CORE


FEATURE_COLUMNS = list(SHARED.FEATURE_COLUMNS)
FOCUS_FEATURES = list(SHARED.FOCUS_FEATURES)
DEFAULT_DATA_PATH = SHARED.DEFAULT_DATA_PATH
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "results" / "base" / "fair_compare" / "cnn_lstm"
DEFAULT_PTL_REFERENCE_DIR = SHARED.DEFAULT_PTL_REFERENCE_DIR
DEFAULT_LOSS_FEATURE_WEIGHTS = dict(SHARED.DEFAULT_LOSS_FEATURE_WEIGHTS)
DEFAULT_MONITOR_FEATURE_WEIGHTS = dict(SHARED.DEFAULT_MONITOR_FEATURE_WEIGHTS)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Daily CNN-LSTM baseline")
...
def main():
    ...
    model = CNN_LSTM_MODEL.DailyCNNLSTMBaseline(
    ...
    run_dir = args.output_root / f"cnn_lstm_{args.station_name}_seed{args.seed}_{timestamp}"
    ...
    print("=" * 70)
    print(f"CNN-LSTM baseline complete: {run_dir}")
    print(
