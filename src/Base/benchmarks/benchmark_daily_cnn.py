...
PTL_CORE = load_module("ptl_progressive_core_cnn_baseline", REPO_ROOT / "src" / "PTL" / "progressive_core.py")
CNN_MODEL = load_module("cnn_baseline_module", MODELS_DIR / "cnn_baseline.py")
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "results" / "base" / "fair_compare" / "cnn"
...
def parse_args():
    parser = argparse.ArgumentParser(description="基础 CNN 日级 baseline")
    ...
    parser.add_argument("--channels", type=int, nargs="*", default=[64, 128, 128])
    parser.add_argument("--kernel-sizes", type=int, nargs="*", default=[3, 3, 3])
...
def main():
    ...
    model = CNN_MODEL.DailyCNNBaseline(
    ...
    run_dir = args.output_root / f"cnn_{args.station_name}_seed{args.seed}_{timestamp}"
    ...
    print("=" * 70)
    print(f"CNN baseline 完成: {run_dir}")
    print(
