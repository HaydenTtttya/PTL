...
PTL_CORE = load_module("ptl_progressive_core_mlp_baseline", REPO_ROOT / "src" / "PTL" / "progressive_core.py")
MLP_MODEL = load_module("mlp_baseline_module", MODELS_DIR / "mlp_baseline.py")
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "results" / "base" / "fair_compare" / "mlp"
...
def parse_args():
    parser = argparse.ArgumentParser(description="基础 MLP 日级 baseline")
...
def main():
    ...
    model = MLP_MODEL.DailyMLPBaseline(
    ...
    run_dir = args.output_root / f"mlp_{args.station_name}_seed{args.seed}_{timestamp}"
    ...
    print("=" * 70)
    print(f"MLP baseline 完成: {run_dir}")
    print(
