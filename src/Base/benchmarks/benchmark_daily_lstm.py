...
BASE_BENCH = load_module("daily_mlp_benchmark_for_lstm", SCRIPT_DIR / "benchmark_daily_mlp.py")
LSTM_MODEL = load_module("lstm_baseline_module", MODELS_DIR / "lstm_baseline.py")
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "results" / "base" / "fair_compare" / "lstm"
DEFAULT_BILSTM_OUTPUT_ROOT = REPO_ROOT / "results" / "base" / "fair_compare" / "bilstm"
...
def parse_args(
    ...
    description: str = "Daily LSTM baseline",
):
    ...
    parser.add_argument("--bidirectional", action=argparse.BooleanOptionalAction, default=default_bidirectional)
...
def main(
    ...
    args = parse_args(
        default_bidirectional=default_bidirectional,
    ...
    model_label = "BiLSTMBaseline" if args.bidirectional else "LSTMBaseline"
    run_prefix = "bilstm" if args.bidirectional else "lstm"
    model_kwargs = {
    ...
    if args.bidirectional:
        model = LSTM_MODEL.DailyBiLSTMBaseline(**model_kwargs)
    else:
        model = LSTM_MODEL.DailyLSTMBaseline(**model_kwargs, bidirectional=False)
    ...
    run_dir = args.output_root / f"{run_prefix}_{args.station_name}_seed{args.seed}_{timestamp}"
    ...
    print("=" * 70)
    print(f"{model_label} complete: {run_dir}")
    print(
