...
PTL_CORE = load_module("ptl_progressive_core_direct_baseline", REPO_ROOT / "src" / "PTL" / "progressive_core.py")
BASELINE_MODEL = load_module("base_direct_compare_baseline", MODELS_DIR / "direct_compare_baseline.py")
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "results" / "base" / "fair_compare" / "daily_direct_baseline"
DEFAULT_PRETRAIN_DIR = (
    ...
    / "pretrain"
    / "v2_full_runs"
    / "pretrain_cross_station_v2_yangzte_seed42_20260406_104617"
)
...
def load_pretrain_config(pretrain_dir: Path):
...
def build_model(args, pretrain_config, feature_columns: list[str]):
    return BASELINE_MODEL.DirectDailyTransformerBaseline(
        num_heads=int(pretrain_config["num_heads"]),
        e_layer=int(pretrain_config["e_layer"]),
        hidden_size=int(pretrain_config["hidden_size"]),
        input_dim=len(feature_columns),
        seq_len=args.model_seq_len,
        pred_len=args.pred_len,
        target_dim=len(feature_columns),
        target_feature_names=feature_columns,
        use_temporal_adapter=args.use_temporal_adapter,
        temporal_adapter_kernel_size=args.temporal_adapter_kernel_size,
    )


def load_matching_pretrain_weights(model, pretrain_dir: Path):
    model_path = pretrain_dir / "model.pth"
    if not model_path.exists():
        raise FileNotFoundError(f"缺少预训练权重: {model_path}")
    pretrain_state = torch.load(model_path, map_location="cpu")
    matched = PTL_CORE.load_matching_weights(model, pretrain_state)
    return matched
...
def build_comparison_summary(base_metrics, reference_metrics, focus_features: list[str]):
    summary = {
...
def parse_args():
    parser = argparse.ArgumentParser(description="用于对比 PTL 的直接日级 Transformer baseline")
    parser.add_argument("--data-path", type=Path, default=DEFAULT_DATA_PATH)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--pretrain-dir", type=Path, default=DEFAULT_PRETRAIN_DIR)
    ...
    parser.add_argument("--lr-milestones", type=int, nargs="*", default=[12, 18])
    ...
    parser.add_argument("--load-pretrain", action=argparse.BooleanOptionalAction, default=True)
...
def main():
    ...
    pretrain_config = load_pretrain_config(args.pretrain_dir)
    model = build_model(args, pretrain_config, feature_columns)
    matched_pretrain = {}
    if args.load_pretrain:
        matched_pretrain = load_matching_pretrain_weights(model, args.pretrain_dir)
    ...
    model, history, best_stats = PTL_CORE.fit_model(
        ...
        log_prefix="DirectDailyBaseline",
        loss_name="mse_nse",
    ...
    mode_name = "direct_daily_pretrained" if args.load_pretrain else "target_only_transformer"
    run_dir = args.output_root / f"{mode_name}_{args.station_name}_seed{args.seed}_{timestamp}"
    ...
    meta = {
        ...
        "load_pretrain": bool(args.load_pretrain),
        "ptl_reference_dir": str(args.ptl_reference_dir) if args.ptl_reference_dir else None,
        "matched_pretrain_keys": int(len(matched_pretrain)),
        "device": str(device),
    ...
    print("=" * 70)
    print(f"Direct daily baseline 完成: {run_dir}")
    print(f"load_pretrain={args.load_pretrain} | matched_pretrain_keys={len(matched_pretrain)}")
    print(
