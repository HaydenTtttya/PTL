...
def main(pretrain_model_dir, custom_config=None, seed=42):
    ...
    print(f">>> 微调超参数: lr={config.base_lr}, freeze_ratio={config.freeze_ratio}")
    ...
    print(f">>> 目标站点: {config.target_station_id}")
    ...
    # 保存模型权重默认关闭，避免微调 run 反复生成大体积 .pth。
    ...
    print(f"模型权重: {'已保存' if model_weights_saved else '未保存'}")
    print(f"\n整体指标:")
    for k, v in metrics['__overall__'].items():
        print(f"  {k}: {v:.4f}")
    print(f"\n结果已保存至: {save_dir}")
...
if __name__ == '__main__':
    ...
    if pretrain_dir is None:
        ...
        best_val_loss, save_path = main(pretrain_model_dir=pretrain_dir, seed=42)
    ...
    #     }
    #     val_loss, _ = main(pretrain_model_dir=pretrain_dir, custom_config=config, seed=42)
    #     return -val_loss
    #
