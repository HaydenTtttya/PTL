...
class CrossStationPretrainConfig:
    ...
    save_root: str = str(REPO_ROOT / "results" / "cross_station" / "pretrain" / "runs")
    ...
    def run_prefix(self) -> str:
        return f"pretrain_cross_station_{self.basin_key}_seed"
...
class CrossStationBackbonePretrainer(nn.Module):
    def __init__(self, backbone, cross_station_heads, cross_station_layers, dropout):
        super().__init__()
    ...
    def forward(self, masked_windows, target_windows, masked_positions, station_available):
        ...
        station_tokens = encoded_stations.permute(0, 2, 1, 3).reshape(
...
def build_masked_inputs(targets, station_available, mask_ratio, mask_strategy, sample_indices=None, base_seed=None):
...
def compute_reconstruction_loss(
    ...
    valid_error = squared_error[valid_positions]
    all_mse = (
        torch.mean(valid_error)
        if valid_error.numel() > 0
        else torch.zeros((), device=preds.device, dtype=preds.dtype)
    )
    masked_valid = masked_positions & valid_positions
    masked_error = squared_error[masked_valid]
    masked_mse = (
    ...
    loss = (all_loss_weight * all_mse) + (masked_loss_weight * masked_mse)
    return loss, {
        "loss": float(loss.detach().cpu()),
        "all_mse": float(all_mse.detach().cpu()),
...
def train_one_epoch(model, loader, optimizer, device, config):
    ...
    for batch_idx, (targets, station_available, sample_indices) in enumerate(loader):
        ...
        optimizer.zero_grad()
        preds = model(masked_inputs, targets, masked_positions, station_available)
        loss, metrics = compute_reconstruction_loss(
            preds,
            targets,
            station_available,
            masked_positions,
            masked_loss_weight=config.masked_loss_weight,
            all_loss_weight=config.all_loss_weight,
        )
        loss.backward()
    ...
    return {
        ...
        "all_mse": total_all_mse / num_batches,
...
def save_run_artifacts(
    ...
    config_payload = {
        **asdict(config),
        ...
        "best_val_all_mse": best_stats["val_all_mse"],
        "best_val_masked_mse": best_stats["val_masked_mse"],
        "checkpoint_format": "merged_ptl_base",
        "ptl_compatible": True,
...
def main(seed=42, cli_args=None):
    ...
    print("=" * 80)
    print(f"seed={seed} basin={config.basin_key} raw_seq_len={config.raw_seq_len} model_seq_len={config.model_seq_len}")
    ...
    backbone = WaterQualityTransformer(
    ...
    best_stats = {
        "epoch": 0,
        "val_loss": float("inf"),
        "val_all_mse": float("inf"),
        "val_masked_mse": float("inf"),
    }
    ...
    for epoch in range(config.pretrain_epochs):
        ...
        history_entry = {
            ...
            "train_all_mse": train_stats["all_mse"],
            "train_masked_mse": train_stats["masked_mse"],
            "train_masked_fraction": train_stats["masked_fraction"],
            "val_loss": val_stats["loss"],
            "val_all_mse": val_stats["all_mse"],
            "val_masked_mse": val_stats["masked_mse"],
    ...
    merged_state = export_merged_compatible_state_dict(
        best_state,
        num_stations=len(station_names),
