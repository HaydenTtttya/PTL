...
class CrossStationPretrainConfig:
    ...
    station_pair_bias_enabled: bool = True
    exclude_self_for_cross_queries: bool = True
    fusion_gate_hidden_size: int | None = None
    ...
    save_root: str = str(REPO_ROOT / "results" / "cross_station" / "pretrain" / "v3_runs")
    ...
    def run_prefix(self) -> str:
        ...
        return f"pretrain_cross_station_v3{backbone_tag}_{self.basin_key}_seed"
...
class CrossStationInteractionBlock(nn.Module):
    def __init__(
        ...
        dropout,
        num_stations,
        pair_bias_enabled=True,
        exclude_self_for_cross_queries=True,
    ):
        super().__init__()
        ...
        self.exclude_self_for_cross_queries = bool(exclude_self_for_cross_queries)
        ...
        if pair_bias_enabled:
            self.station_pair_bias = nn.Parameter(torch.zeros(self.num_stations, self.num_stations))
            self.station_pair_scale = nn.Parameter(torch.ones(1))
        else:
            self.station_pair_bias = None
            self.station_pair_scale = None

    def forward(self, x, key_padding_mask=None, query_update_mask=None):
        batch_size, num_stations, _ = x.shape
        q = self.q_proj(x).view(batch_size, num_stations, self.num_heads, self.head_dim).transpose(1, 2)
        k = self.k_proj(x).view(batch_size, num_stations, self.num_heads, self.head_dim).transpose(1, 2)
        v = self.v_proj(x).view(batch_size, num_stations, self.num_heads, self.head_dim).transpose(1, 2)

        logits = torch.matmul(q, k.transpose(-2, -1)) * self.scale
        if self.station_pair_bias is not None:
            logits = logits + (
                self.station_pair_scale
                * self.station_pair_bias.view(1, 1, self.num_stations, self.num_stations)
            )

        if key_padding_mask is not None:
            logits = logits.masked_fill(key_padding_mask[:, None, None, :], float("-inf"))

        if query_update_mask is not None and self.exclude_self_for_cross_queries:
            if key_padding_mask is None:
                available_counts = torch.full(
                    (batch_size, num_stations),
                    fill_value=num_stations,
                    device=x.device,
                    dtype=torch.long,
                )
            else:
                available_counts = (~key_padding_mask).sum(dim=1, keepdim=True).expand(-1, num_stations)
            can_exclude_self = query_update_mask & (available_counts > 1)
            eye = torch.eye(num_stations, device=x.device, dtype=torch.bool).view(1, 1, num_stations, num_stations)
            logits = logits.masked_fill(can_exclude_self[:, None, :, None] & eye, float("-inf"))

        attention = torch.softmax(logits, dim=-1)
        attention = torch.nan_to_num(attention, nan=0.0, posinf=0.0, neginf=0.0)
        attention = self.attention_dropout(attention)
        attended = torch.matmul(attention, v).transpose(1, 2).contiguous().view(batch_size, num_stations, self.hidden_size)
        attended = self.out_proj(attended)

        if query_update_mask is not None:
            update_mask = query_update_mask.unsqueeze(-1).to(attended.dtype)
            attended = attended * update_mask
        x = self.norm1(x + attended)

        ffn_out = self.ffn(x)
        if query_update_mask is not None:
            ffn_out = ffn_out * update_mask
...
class CrossStationBackbonePretrainer(nn.Module):
    def __init__(
        ...
        station_pair_bias_enabled=True,
        exclude_self_for_cross_queries=True,
        fusion_gate_hidden_size=None,
    ):
        super().__init__()
        ...
        self.uses_generic_pretrain_interface = all(
            ...
            for attribute in (
                "encode_pretrain_tokens",
                "decode_pretrain_tokens",
                "build_cross_delta_decoder",
            )
        )
        self.cross_station_layers = nn.ModuleList(
            [
                CrossStationInteractionBlock(
                    ...
                    pair_bias_enabled=station_pair_bias_enabled,
                    exclude_self_for_cross_queries=exclude_self_for_cross_queries,
                )
        ...
        gate_hidden = (
            int(fusion_gate_hidden_size)
            if fusion_gate_hidden_size is not None
            else max(32, backbone.d_model // 2)
        )
        self.cross_delta_head = (
            backbone.build_cross_delta_decoder()
            if self.uses_generic_pretrain_interface
            else copy.deepcopy(backbone.head)
        )
        self.fusion_gate = nn.Sequential(
            nn.Linear(backbone.d_model + 2, gate_hidden),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(gate_hidden, backbone.target_dim),
        )
    ...
    def decode_delta_station(self, encoded_delta, stdev):
        if self.uses_generic_pretrain_interface:
            return self.backbone.decode_pretrain_tokens(
                encoded_delta,
                decoder=self.cross_delta_head,
            )

        decoded = self.cross_delta_head(encoded_delta).permute(0, 2, 1)
        decoded = decoded[:, :, : self.backbone.target_dim]
        target_stdev = stdev[:, 0, : self.backbone.target_dim].unsqueeze(1).repeat(
            1,
            self.backbone.pred_len,
        ...
        return decoded * target_stdev

    def build_station_gate(self, encoded_stations, station_mask_fraction, station_available):
        station_summary = encoded_stations.mean(dim=2)
        gate_features = torch.cat(
            [
                station_summary,
                station_mask_fraction.unsqueeze(-1),
                station_available.float().unsqueeze(-1),
            ],
            dim=-1,
        )
        gate = torch.sigmoid(self.fusion_gate(gate_features))
        gate = gate * station_mask_fraction.gt(0).unsqueeze(-1).float()
        gate = gate * station_available.float().unsqueeze(-1)
        return gate
    ...
    def forward(self, masked_windows, target_windows, masked_positions, station_available):
        ...
        means_list = []
        stdev_list = []
        station_mask_fraction = masked_positions.float().mean(dim=(2, 3))
        station_needs_cross = station_available.bool() & station_mask_fraction.gt(0)
        ...
        for layer in self.cross_station_layers:
            station_context = layer(
                station_context,
                key_padding_mask=key_padding_mask,
                query_update_mask=query_update_mask,
            )

        cross_delta_tokens = (station_context - base_station_tokens).reshape(
            batch_size,
            feature_token_count,
            num_stations,
            self.backbone.d_model,
        ).permute(0, 2, 1, 3)
        station_gate = self.build_station_gate(
            encoded_stations=encoded_stations,
            station_mask_fraction=station_mask_fraction,
        ...
        for station_idx in range(num_stations):
            cross_deltas.append(
                self.decode_delta_station(
                    cross_delta_tokens[:, station_idx],
                    stdev_list[station_idx],
                )
            )
        cross_delta_predictions = torch.stack(cross_deltas, dim=1)
        cross_predictions = local_predictions + station_gate.unsqueeze(2) * cross_delta_predictions
...
def save_run_artifacts(
    ...
    config_payload = {
        **asdict(config),
        ...
        "checkpoint_format": (
            "merged_ptl_base" if is_transformer else "forecast_backbone_encoder"
        ),
...
def main(seed=42, cli_args=None):
    ...
    model = CrossStationBackbonePretrainer(
        ...
        station_pair_bias_enabled=config.station_pair_bias_enabled,
        exclude_self_for_cross_queries=config.exclude_self_for_cross_queries,
        fusion_gate_hidden_size=config.fusion_gate_hidden_size,
    ).to(device)
